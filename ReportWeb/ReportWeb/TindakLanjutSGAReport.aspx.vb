...
Partial Class TindakLanjutSGAReport
    ...
    Private Sub LoadData()
        ...
        Try
            ...
            dt = ObjSQL.SQLInsertIntoDatatable(MCon, "call sp_tgf_ddl('PROCESSTYPE')", SqlParam)

            If Not IsNothing(dt) Then
                If dt.Rows.Count > 0 Then
                    ObjFungsi.ddlBound(ddlTindakLanjut, dt, "Display", "Value")

                    dt.DefaultView.RowFilter = "Value <> ''"
                    dt = dt.DefaultView.ToTable
                    ObjFungsi.chkBound(chkList3, dt, "Display", "Value")

                    ViewState("Multi_TindakLanjut_Reset") = dt
                Else
                    ErrorList.AppendLine("getTindakLanjut Error!")
                End If
            Else
                ErrorList.AppendLine("getTindakLanjut Error!")
            End If
            ...
            SqlParam = New Dictionary(Of String, String)
            dt = ObjSQL.SQLInsertIntoDatatable(MCon, "call sp_tgf_ddl('STATUS')", SqlParam)
            ...
            If Not IsNothing(dt) Then
                ...
                Else
                    ErrorList.AppendLine("getStatus Error!")
                End If
            Else
                ErrorList.AppendLine("getStatus Error!")
            End If
    ...
    Protected Sub ddlStatus_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlStatus.SelectedIndexChanged
        ...
        If ddlStatus.SelectedValue = "TGF" Then
            ddlTindakLanjut.Enabled = True
            btnMulti3.Enabled = True
        Else
            ddlTindakLanjut.Enabled = False
            btnMulti3.Enabled = False

            td3.Visible = True
            tr3.Visible = False

            ddlTindakLanjut.SelectedValue = ""

            ObjFungsi.DeSelectAll(chkList3)
            ViewState("Multi_TindakLanjut") = ObjFungsi.SetDataMulti(btnMulti3, chkList3)
            ViewState("Multi_TindakLanjutName") = ObjFungsi.SetDataMultiName(btnMulti3, chkList3)

        End If

    End Sub

    Private Function ValidasiProses(ByRef NoTGF As String) As Boolean
        ...
        Try
            ...
            NoTGF = txtNoTGF.Text.Trim.Replace(vbCrLf, "/")
            Dim TGF() As String = NoTGF.Split(New String() {"|"}, StringSplitOptions.RemoveEmptyEntries)

            If TGF.Length > 10 Then
                lblError.Text = "Maksimal Jumlah Nomor 10 !!"
                Return False
            End If

            If TGF.Length <= 1 Then
    ...
    Private Sub ProsesPreview(ByVal PreviewOnly As Boolean)
        ...
        Try
            If ValidasiProses(NoTGF) Then
                Dim KodeTindakLanjut As String = GetTindakLanjut("CODE")
                Dim NamaTindakLanjut As String = GetTindakLanjut("NAME")
                Dim StatusCode As String = ddlStatus.SelectedValue
                Dim StatusName As String = ddlStatus.SelectedItem.Text
                ...
                Proses(KodeTindakLanjut, NamaTindakLanjut, StatusCode, StatusName, TanggalAwal, TanggalAkhir, NoTGF, KodeEcommerce, NamaEcommerce, TipeSeller, TipeSellerName, SubCategorySeller, SubCategorySellerName, KodeLayanan, NamaLayanan, PreviewOnly)
    ...
    Private Sub Proses(ByVal KodeTindakLanjut As String, ByVal NamaTindakLanjut As String, ByVal StatusCode As String, ByVal StatusName As String _
    , ByVal TanggalAwal As String, ByVal TanggalAkhir As String, ByVal NoTGF As String, ByVal KodeEcommerce As String, ByVal NamaEcommerce As String, ByVal TipeSeller As String, ByVal TipeSellerName As String, ByVal SubCategorySeller As String, ByVal SubCategorySellerName As String, ByVal KodeLayanan As String, ByVal NamaLayanan As String, ByVal PreviewOnly As Boolean)
        ...
        If User <> "" Then
            ...
            Try
                ...
                SqlQuery &= " , @Type"
                SqlQuery &= " , @TipeSeller"
                SqlQuery &= " , @SubCategorySeller"
                SqlQuery &= " , @KodeEcommerce"
                SqlQuery &= " , @KodeLayanan"
                SqlQuery &= " , @Preview"
                SqlQuery &= " )"
                ...
                SqlParam.Add("@TrackNum", NoTGF)
                SqlParam.Add("@Account", "")
                SqlParam.Add("@FollowUp", KodeTindakLanjut)
                SqlParam.Add("@Status", StatusCode)
                SqlParam.Add("@Type", "REPORT")
                ...
                If PreviewOnly Then
                    ...
                    HeaderTitle(6) = "STATUS"
                    HeaderTitle(7) = "STATUS TINDAK LANJUT"
                    HeaderTitle(8) = "AWB IPP"
                    ...
                    HeaderContent(6) = StatusName
                    HeaderContent(8) = NoTGF

                    Session("TitleReport") = "LAPORAN TINDAK LANJUT SGA"
                ...
                Else
                    ...
                    Dim FileName As String = "TGFREPORT_" & AutoNum
...
#Region "Button Multi Select"
    ...
    Private Function GetTindakLanjut(ByVal Tipe As String) As String
        ...
        Try
            ...
            Else
                If hasil2(0) = "0" Or hasil2(0) = "2" Then
                    If Tipe = "NAME" Then
                        hasil1 = ddlTindakLanjut.SelectedItem.Text
                    Else
                        hasil1 = ddlTindakLanjut.SelectedValue
                    End If
                Else
                    If Tipe = "NAME" Then
                        hasil1 = ViewState("Multi_TindakLanjutName")
                    Else
                        hasil1 = ViewState("Multi_TindakLanjut")
                    End If
                End If
            End If
    ...
    Protected Sub btnMulti1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnMulti3.Click, btnMulti40.Click, btnMulti51.Click, btnMulti61.Click, btnMulti7.Click
        ...
        If btnID = "3" Then
            td3.Visible = False
            tr3.Visible = True
            ObjFungsi.SetMulti(chkList3, ViewState("Multi_TindakLanjut"))
    ...
    Protected Sub btnAll1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAll3.Click, btnAll40.Click, btnAll51.Click, btnAll61.Click, btnAll7.Click
    ...
    Protected Sub btnNon1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNon3.Click, btnNon40.Click, btnNon51.Click, btnNon61.Click, btnNon7.Click
    ...
    Protected Sub btnBatal1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBatal3.Click, btnBatal40.Click, btnBatal51.Click, btnBatal61.Click, btnBatal7.Click
    ...
    Protected Sub btnPilih1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPilih3.Click, btnPilih40.Click, btnPilih51.Click, btnPilih61.Click, btnPilih7.Click
